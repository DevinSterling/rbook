...
- New options for `EpubOpenOptions` for speed/space optimization:
  - `skip_metadata`: Toggle to skip parsing metadata.
  - `skip_manifest`: Toggle to skip parsing the manifest.
  - `skip_spine`: Toggle to skip parsing the spine.
  - `skip_toc`: Toggle to skip parsing `.ncx/.xhtml` table of contents files.

### Changes　**⟳**
- Make parsing more lenient when `EpubOpenOptions::strict` is set to `false`.
